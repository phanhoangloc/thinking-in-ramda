...
Bài viết này là Phần 6 của loạt bài về lập trình hàm được gọi là Thinking in Ramda.

Trong phần 5, chúng ta đã nói về cách viết các hàm của chúng ta theo kiểu "pointfree" hoặc "tacit", khi màtham đối số chính của hàm không được hiển thị rõ ràng.

Vào thời điểm đó, chúng ta đã không thể chuyển đổi tất cả các hàm thành pointfree bởi vì chúng ta đã thiếu một số công cụ. Đã đến lúc học các công cụ này.
...
Để làm cho các hàm này pointfree, chúng ta cần một cách để xây dựng một hàm mà sau đó chúng ta có thể áp dụng cho người cuối cùng. Vấn đề là chúng ta cần truy cập vào tài sản trên người và cách duy nhất chúng ta biết làm thế nào là bắt buộc.

chống đỡ

May mắn thay, Ramda có thể giúp chúng tôi. Nó cung cấp chức năng chống đỡ để truy cập các thuộc tính của một đối tượng.
...
Wow, trông bây giờ tệ hơn nhiều. Nhưng chúng ta hãy tiếp tục với refactoring. Thứ nhất, chúng ta hãy hoán đổi thứ tự của các đối số mà chúng ta đang đi ngang bằng nhau để prop đến cuối cùng. bằng công trình tương tự với một trong hai thứ tự, vì vậy điều này là an toàn.
...
Tiếp theo, chúng ta hãy sử dụng tính chất curried của equals và gte để làm cho các chức năng mới mà chúng ta áp dụng cho kết quả của các cuộc gọi prop.
...
Điều đó vẫn còn tồi tệ hơn một chút, nhưng hãy tiếp tục đi nào. Hãy tận dụng lợi thế của currying một lần nữa với tất cả các cuộc gọi prop.
...
Tệ hơn nữa. Nhưng bây giờ chúng ta thấy một khuôn mẫu quen thuộc. Cả ba chức năng của chúng ta có cùng hình dạng với f \(g \(người\)\), và chúng ta biết từ Phần 2 rằng điều này tương đương với soạn \(f, g\) \(người\).
...
Bây giờ chúng ta đang đi đâu đó. Bây giờ tất cả ba chức năng trông giống như người =&gt; f \(người\). Chúng ta biết từ Phần 5 rằng chúng ta có thể làm cho các chức năng này pointfree.
...
Không rõ ràng khi chúng tôi bắt đầu rằng các phương pháp của chúng tôi đã làm hai việc khác nhau. Cả hai đều đang truy cập tài sản của một đối tượng và thực hiện một số hoạt động về giá trị của tài sản đó. Việc tái cấu trúc này thành phong cách không pointfree đã làm cho điều đó rất rõ ràng.
...
Trường hợp prop đọc một thuộc tính từ một đối tượng và trả về giá trị, chọn đọc nhiều thuộc tính từ một đối tượng và trả về một đối tượng mới chỉ với các thuộc tính đó.



Ví dụ: nếu muốn chỉ tên và tuổi của một người, chúng tôi có thể sử dụng chọn \(\['tên', 'tuổi'\), người\).



có

Nếu chúng ta chỉ muốn biết nếu một đối tượng có một thuộc tính không đọc giá trị thì chúng ta có thể sử dụng để kiểm tra thuộc tính của chính nó, và hasIn để kiểm tra chuỗi nguyên mẫu: has \('name', person\).

con đường

Trường hợp prop đọc một thuộc tính từ một đối tượng, đường đi lặn vào các đối tượng lồng nhau. Ví dụ: chúng tôi có thể truy cập mã zip từ cấu trúc sâu hơn dưới dạng đường dẫn \(\['address', 'zipCode'\), person\).



Lưu ý rằng đường đi là tha thứ hơn prop. con đường sẽ trở lại không xác định nếu bất cứ điều gì dọc theo con đường \(bao gồm cả các đối số ban đầu\) là null hoặc undefined trong khi prop sẽ gây ra một lỗi.
...
Ví dụ: chúng tôi có thể cung cấp trình giữ chỗ khi chúng tôi không biết tên của một người: propOr \('&lt;Unnamed&gt;', 'tên', người\). Lưu ý rằng không giống như chống đỡ, propOr sẽ không gây ra một lỗi nếu người là vô giá trị hoặc không xác định; nó sẽ thay vì trả về giá trị mặc định.
